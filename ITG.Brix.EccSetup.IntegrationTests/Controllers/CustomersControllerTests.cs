...
namespace ITG.Brix.EccSetup.IntegrationTests.Controllers
{
    ...
    public class CustomersControllerTests
    {
        ...
        public async Task ListAllShouldSucceed()
        {
            // Arrange
            var apiVersion = "1.0";
            await ControllerHelper.CreateCustomer("TestCode", "TestName", "TestSource");

            // Act
            var response = await _client.GetAsync(string.Format("api/customers?api-version={0}", apiVersion));
            var responseBody = await response.Content.ReadAsStringAsync();
            var businessUnitsModel = JsonConvert.DeserializeObject<CustomersModel>(responseBody);
        ...
        public async Task ListWithFilterShouldSucceed(string filter, int countResult)
        {
            ...
            {
                await ControllerHelper.CreateCustomer(code + c, name, source);
            }

            // Act
            var response = await _client.GetAsync(string.Format("api/customers?api-version={0}&$filter={1}", apiVersion, filter));
            var responseBody = await response.Content.ReadAsStringAsync();
            var customersModel = JsonConvert.DeserializeObject<CustomersModel>(responseBody);

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);
            customersModel.Value.Should().HaveCount(countResult);
            customersModel.Count.Should().Be(countResult);
            customersModel.NextLink.Should().BeNull();
        }
        ...
        public async Task ListWithSkipAndTopShouldSucceed(int skip, int top, int countResult)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/customers?api-version={0}&$skip={1}&$top={2}", apiVersion, skip, top));
            var responseBody = await response.Content.ReadAsStringAsync();
            var locationsModel = JsonConvert.DeserializeObject<CustomersModel>(responseBody);
        ...
        public async Task ListWithFilterSkipTopShouldSucceed(string filter, int skip, int top, int countResult)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/customers?api-version={0}&$filter={1}&$skip={2}&$top={3}", apiVersion, filter, skip, top));
            var responseBody = await response.Content.ReadAsStringAsync();
            var businessUnitsModel = JsonConvert.DeserializeObject<CustomersModel>(responseBody);
        ...
        public async Task ListShouldSucceedWhenQueryTopPresentButUnset(string top)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/customers?api-version={0}&$top={1}", apiVersion, top));
        ...
        public async Task ListShouldSucceedWhenQuerySkipPresentButUnset(string skip)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/customers?api-version={0}&$skip={1}", apiVersion, skip));
        ...
        public async Task CreateShouldFailWhenBodyIsNonJsonContentType()
        {
            ...
            // Act
            var response = await _client.PostAsync(string.Format("api/customers?api-version={0}", apiVersion), new StringContent(jsonBody, Encoding.UTF8, "text/plain"));
            var responseBody = await response.Content.ReadAsStringAsync();
        ...
        public async Task CreateShouldFailWhenQueryApiVersionIsMissing()
        {
            ...
            // Act
            var response = await _client.PostAsync("api/customers", new StringContent(jsonBody, Encoding.UTF8, "application/json"));
            var responseBody = await response.Content.ReadAsStringAsync();
            ...
            var expectedError = new ResponseError
            {
                Error = new ResponseErrorBody
                {
                    ...
                    Details = new List<ResponseErrorField>
                    {
                        ...
                        {
                            Code = "invalid-input",
                            Message = ValidationFailures.CustomerCodeMandatory,
                            Target = "code"
                        }
                    }
                }
            ...
            var expectedError = new ResponseError
            {
                Error = new ResponseErrorBody
                {
                    ...
                    Details = new List<ResponseErrorField>
                    {
                        ...
                        {
                            Code = "invalid-input",
                            Message = ValidationFailures.CustomerSourceMandatory,
                            Target = "source"
                        }
                    }
                }
        ...
        public async Task CreateShouldFailWhenCodeAndSourceAlreadyExists()
        {
            ...
            var apiVersion = "1.0";
            await ControllerHelper.CreateCustomer(code, name, source);
