...
namespace ITG.Brix.EccSetup.IntegrationTests.Controllers
{
    ...
    public class ProductionSitesControllerTests
    {
        ...
        {
            DatabaseHelper.Init("ProductionSites");
        ...
        public async Task ListAllShouldSucceed()
        {
            // Arrange
            var apiVersion = "1.0";
            await ControllerHelper.CreateProductionSite("TestCode", "TestName", "TestSource");

            // Act
            var response = await _client.GetAsync(string.Format("api/productionsites?api-version={0}", apiVersion));
            var responseBody = await response.Content.ReadAsStringAsync();
            var productionSitesModel = JsonConvert.DeserializeObject<ProductionSitesModel>(responseBody);

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);
            productionSitesModel.Value.Should().HaveCount(1);
            productionSitesModel.Count.Should().Be(1);
            productionSitesModel.NextLink.Should().BeNull();
        }
        ...
        public async Task ListShouldFailWhenQueryApiVersionIsMissing()
        {
            ...
            // Act
            var response = await _client.GetAsync("api/productionsites");
        ...
        public async Task ListWithFilterShouldSucceed(string filter, int countResult)
        {
            ...
            {
                await ControllerHelper.CreateProductionSite(code + c, name, source);
            }

            // Act
            var response = await _client.GetAsync(string.Format("api/productionsites?api-version={0}&$filter={1}", apiVersion, filter));
            var responseBody = await response.Content.ReadAsStringAsync();
            var productionSitesModel = JsonConvert.DeserializeObject<ProductionSitesModel>(responseBody);

            // Assert
            response.StatusCode.Should().Be(HttpStatusCode.OK);
            productionSitesModel.Value.Should().HaveCount(countResult);
            productionSitesModel.Count.Should().Be(countResult);
            productionSitesModel.NextLink.Should().BeNull();
        }
        ...
        public async Task ListWithSkipAndTopShouldSucceed(int skip, int top, int countResult)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/productionsites?api-version={0}&$skip={1}&$top={2}", apiVersion, skip, top));
            var responseBody = await response.Content.ReadAsStringAsync();
            var productionSitesModel = JsonConvert.DeserializeObject<ProductionSitesModel>(responseBody);
        ...
        public async Task ListWithFilterSkipTopShouldSucceed(string filter, int skip, int top, int countResult)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/productionsites?api-version={0}&$filter={1}&$skip={2}&$top={3}", apiVersion, filter, skip, top));
            var responseBody = await response.Content.ReadAsStringAsync();
            var productionSitesModel = JsonConvert.DeserializeObject<ProductionSitesModel>(responseBody);
        ...
        public async Task ListShouldSucceedWhenQueryTopPresentButUnset(string top)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/productionsites?api-version={0}&$top={1}", apiVersion, top));
        ...
        public async Task ListShouldSucceedWhenQuerySkipPresentButUnset(string skip)
        {
            ...
            // Act
            var response = await _client.GetAsync(string.Format("api/productionsites?api-version={0}&$skip={1}", apiVersion, skip));
        ...
        public async Task CreateShouldFailWhenBodyIsNonJsonContentType()
        {
            ...
            // Act
            var response = await _client.PostAsync(string.Format("api/productionsites?api-version={0}", apiVersion), new StringContent(jsonBody, Encoding.UTF8, "text/plain"));
            var responseBody = await response.Content.ReadAsStringAsync();
        ...
        public async Task CreateShouldFailWhenQueryApiVersionIsMissing()
        {
            ...
            // Act
            var response = await _client.PostAsync("api/productionsites", new StringContent(jsonBody, Encoding.UTF8, "application/json"));
            var responseBody = await response.Content.ReadAsStringAsync();
            ...
            var expectedError = new ResponseError
            {
                Error = new ResponseErrorBody
                {
                    ...
                    Details = new List<ResponseErrorField>
                    {
                        ...
                        {
                            Code = "invalid-input",
                            Message = ValidationFailures.ProductionSiteCodeMandatory,
                            Target = "code"
                        }
                    }
                }
            ...
            var expectedError = new ResponseError
            {
                Error = new ResponseErrorBody
                {
                    ...
                    Details = new List<ResponseErrorField>
                    {
                        ...
                        {
                            Code = "invalid-input",
                            Message = ValidationFailures.ProductionSiteSourceMandatory,
                            Target = "source"
                        }
                    }
                }
            ...
            var expectedError = new ResponseError
            {
                Error = new ResponseErrorBody
                {
                    ...
                    Details = new List<ResponseErrorField>
                    {
                        ...
                        {
                            Code = "invalid-input",
                            Message = ValidationFailures.ProductionSiteNameMandatory,
                            Target = "name"
                        }
                    }
                }
        ...
        [TestMethod]
        public async Task CreateShouldFailWhenNameIsWhiteSpace()
        {
            // Arrange
            string name = "   ";
            string code = "TCode";
        ...
        [TestMethod]
        public async Task CreateShouldFailWhenCodeAndNameAndSourceAlreadyExists()
        {
            // Arrange
            string name = "TName";
            string code = "TCode";
            string source = "TSource";
            var apiVersion = "1.0";
            await ControllerHelper.CreateProductionSite(code, name, source);
